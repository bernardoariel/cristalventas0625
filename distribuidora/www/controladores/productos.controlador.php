		...
		if(isset($_POST["nuevaDescripcion"])){
			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ\/.,&*()\[\] ]+$/', $_POST["nuevaDescripcion"]) &&
			   ...
			   preg_match('/^[0-9.]+$/', $_POST["nuevoStock"]) ){
				...
				$datos = array(
				"id_categoria"   => $_POST["nuevaCategoria"] ?? 0,
				"codigo"         => trim($_POST["nuevoCodigo"] ?? ''),
				"nombre"         => trim(strtoupper($_POST["nuevoNombre"] ?? '')),
				"descripcion"    => trim(strtoupper($_POST["nuevaDescripcion"] ?? '')),
				"stock"          => trim($_POST["nuevoStock"] ?? '0'),
				"precio_compra"  => is_numeric($_POST["nuevoPrecioCompra"] ?? null) ? trim($_POST["nuevoPrecioCompra"]) : '0',
				"precio_venta"   => is_numeric($_POST["nuevoPrecioVenta"] ?? null) ? trim($_POST["nuevoPrecioVenta"]) : '0',
				"cantminima" => is_numeric($_POST["nuevaCantMinima"] ?? null) ? $_POST["nuevaCantMinima"] : 0,
				"proveedor" => is_numeric($_POST["nuevoProveedor"] ?? null) ? (int)$_POST["nuevoProveedor"] : 0
				);
