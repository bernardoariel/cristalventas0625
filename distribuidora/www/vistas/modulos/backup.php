       ...
       <table class="table table-bordered table-striped dt-responsive tablas" width="100%">
        ...
        <?php
          ...
          foreach ($backups as $key => $value) {
            ...
            $datosViejos = !empty($value["datos_viejos"]) ? json_decode($value["datos_viejos"], true) : [];
            $datosNuevos = !empty($value["datos_nuevos"]) ? json_decode($value["datos_nuevos"], true) : [];

            // Inicializar las variables
            $tablaNViejos = "<ul><li>Sin datos anteriores</li></ul>";
            $tablaNuevos = "<ul><li>Sin datos nuevos</li></ul>";
            ...
            switch ($value["tabla"]) {
              case 'productos':
                ...
                case 'ventas':
                  ...
                  break;
              default:
                // Para cualquier otra tabla, mostrar los datos en formato genérico
                $tablaNViejos = "<ul>";
                foreach($datosViejos as $key => $valor) {
                    $tablaNViejos .= "<li>".$key.": ".$valor."</li>";
                }
                $tablaNViejos .= "</ul>";

                $tablaNuevos = "<ul>";
                foreach($datosNuevos as $key => $valor) {
                    $tablaNuevos .= "<li>".$key.": ".$valor."</li>";
                }
                $tablaNuevos .= "</ul>";
                break;
            }
